...
=item parent_encoding_name => $name

The encoding name or alias, provided by the context in which the
reference to the style sheet appears.  If the style sheet is
referenced by a C<link> element or C<xml-stylseheet> processing
instruction, its C<charset> attribute value should be specified here.
Otherwise, the character encoding of the document referencing the
style sheet directly or indirectly should be specified here.  See the
CSS specification [CSSSYNTAX] for details.  If no such contextual
character encoding metadata is available, this option has to be left
undefined (or C<undef>).

=back

If the style sheet is provided by a non-HTTP data source and you know
the encoding name a priori, it should be specified as
C<transport_encoding_name>.

The encoding name acutally used to parse the CSS style sheet is
available as C<< $ss->{input_encoding} >>.  (If the style sheet is
created from a character string by one of following methods, this
