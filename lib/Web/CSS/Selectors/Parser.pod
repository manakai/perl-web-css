...
the null namespace).  The second argument (item of index 2) is the
namespace prefix.  The namespace prefix is the empty string if the
default namespace is used explicitly by type or universal selector, or
implicitly by implicit universal selector.  The namespace prefix is
C<undef> if the empty string prefix syntax (representing the null
namespace) is used.
